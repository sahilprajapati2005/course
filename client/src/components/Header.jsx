            ...
            {isMenuOpen && (
                <div className="md:hidden bg-white border-t border-gray-200">
                    <nav className="px-2 pt-2 pb-4 space-y-1 sm:px-3">
                        <NavLink to="/" className={({ isActive }) => `block px-3 py-2 rounded-md text-base font-medium ${isActive ? 'text-white bg-indigo-600' : 'text-gray-700 hover:bg-gray-100'}`}>Home</NavLink>
                        {isAdmin && (
                            <>
                                <NavLink to="/admin/dashboard" className={({ isActive }) => `block px-3 py-2 rounded-md text-base font-medium ${isActive ? 'text-white bg-indigo-600' : 'text-gray-700 hover:bg-gray-100'}`}>Dashboard</NavLink>
                                <NavLink to="/admin/add-course" className={({ isActive }) => `block px-3 py-2 rounded-md text-base font-medium ${isActive ? 'text-white bg-indigo-600' : 'text-gray-700 hover:bg-gray-100'}`}>Add Course</NavLink>
                            </>
                        )}
                        {user && !isAdmin && (
                             <NavLink to="/user/dashboard" className={({ isActive }) => `block px-3 py-2 rounded-md text-base font-medium ${isActive ? 'text-white bg-indigo-600' : 'text-gray-700 hover:bg-gray-100'}`}>My Courses</NavLink>
                        )}
