...
const createOrder = async (req, res) => {
    try {
        ...
        res.status(201).json({ success: true, orderId: order.id, amount: course.price });
    } catch (error) {
...
const verifyPayment = async (req, res) => {
    try {
        ...
        await User.findByIdAndUpdate(enrollment.user, { $push: { enrolledCourses: enrollment._id } });

        res.status(200).json({ success: true, message: 'Payment verified and course unlocked!', enrollment });
    } catch (error) {
